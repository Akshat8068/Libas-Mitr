...
const AdminProduct = () => {
    ...
    const {name,description,brand,categories,originalPrice,salePrice,colors}=productForm
    ...
    const {colorName,mainImage,mainImageFile,images,imageFiles, sizes}=currentColor
    ...
    const availableBrands = [
        ...
        "Nike", "Prada", "PeterEngland", "Puma", "Versace", "Generic"
    ];
    const availableSizes = ["S", "M", "L", "XL", "2XL", "3XL"];
   
    // Add this state to track which color is being edited
    ...
    //     });
    // };
    // Modify the addColorToProduct function to handle both add and edit
    const addColorToProduct = () => {
        if (!colorName) {
            toast.error('Please enter a color name');
            return;
        ...
        if (editingColorIndex !== null) {
            ...
            setProductForm(prev => ({
                ...
                colors: prev.colors.map((color, idx) =>
                    idx === editingColorIndex ? { ...currentColor } : color
                )
            }));
            toast.success(`${currentColor.colorName} color updated!`);
    ...
    // Add function to edit a color
    const handleEditColor = (index) => {
        const colorToEdit = colors[index];
        setCurrentColor({
            colorName: colorToEdit.colorName,
            mainImage: colorToEdit.mainImage,
            mainImageFile: colorToEdit.mainImageFile || null,
            images: colorToEdit.images || [],
            imageFiles: colorToEdit.imageFiles || [],
            sizes: colorToEdit.sizes || [{ size: 'S', stock: 0 }]
        });
        setEditingColorIndex(index);
        toast.info(`Editing ${colorToEdit.colorName} color`);
    };
    ...
    const handleCancel = () => {
        ...
        // ✅ Clear the productEdit state
        dispatch(editProduct({ _id: null }));
        // or create a clearProductEdit action
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation
        if (!name || !description || !brand || !originalPrice || !salePrice) {
        ...
        const colorsData = colors.map(color => ({
            ...
            sizes: color.sizes.map(size => ({
                size: size.size,
                stock: parseInt(size.stock) || 0  // Convert to number
            }))
        }));
    ...
    useEffect(() => {
        ...
        if (productEdit.isEdit && productEdit.product && productEdit.product._id) {
            setProductForm({
                ...productEdit.product,
                categories: productEdit.product.categories || [],  // ✅ Ensure categories is always an array
                colors: productEdit.product.colors || []           // ✅ Ensure colors is always an array
            })
        }

       
    }, [user, adminErrorMessage, adminIsError,productEdit])
                                    ...
                                    <tr key={product._id} className="bg-cover bg-center transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <button to={"/admin/products/:pid"}
                                                    className="p-2 text-violet-600 hover:bg-violet-50 rounded-lg transition-colors"
                                                    title="Edit"
                                                    onClick={() => { setIsModalOpen(true),handleProductEdit(product) }}
                                                >
                                ...
                                {/* Modal Header */}
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {availableCategories.map((cat) => (
                                                    <button
                                                        ...
                                                        className={`px-4 py-2 rounded-lg border-2 font-medium transition-colors ${categories.includes(cat)
                                    ...
                                    {showColorSection && (
                                        <div className="space-y-6 border-t pt-6">
                                            ...
                                            {/* Current Color Being Added/Edited */}
                                            <div className="bg-gray-50 p-4 rounded-lg space-y-4">
                                                    ...
                                                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-violet-500 transition-colors">
                                                        {mainImage ? (
                                                            <div className="relative inline-block">
                                                                <img src={mainImage} alt="Main" className="w-32 h-32 object-cover rounded-lg" />
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setCurrentColor({ ...currentColor, mainImage: null, mainImageFile: null })}
                                                ...
                                                <button
                                                    ...
                                                    className={`w-full px-4 py-2 text-white rounded-lg font-medium transition-colors ${editingColorIndex !== null
                                                            ? 'bg-blue-600 hover:bg-blue-700'
                                                            : 'bg-green-600 hover:bg-green-700'
                                                        }`}
                                                >
                                                    {editingColorIndex !== null ? 'Update This Color' : 'Save This Color'}
                                                </button>
                                            </div>
                                            ...
                                            {colors.length > 0 && (
                                                <div className="space-y-2">
                                                    <h5 className="font-semibold text-gray-800">Added Colors ({colors.length})</h5>
                                                    {colors.map((color, idx) => (
                                                        <div
                                                            key={idx}
                                                            className={`flex items-center justify-between bg-white border p-3 rounded-lg ${editingColorIndex === idx ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                                                                }`}
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                {color.mainImage && (
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <button
                                                                    ...
                                                                    disabled={editingColorIndex !== null && editingColorIndex !== idx}
                                                                >
                                                                    <Edit2 className="w-4 h-4" />
                                                                </button>
                                                                <button
                                                                    ...
                                                                    className="p-2 text-red-600 hover:bg-red-50 rounded-lg"
                                                                    disabled={editingColorIndex !== null}
                                                                >
                                                                    <Trash2 className="w-4 h-4" />
                                                                </button>
                                                            </div>
